function lezah:loading/api/main
function #lezah:tick_triggers
function lezah:old_content_folder/ticks
function lezah:old_content_folder/logros
function lezah:old_content_folder/itemstuff/main
title @a[gamemode=spectator] actionbar [{"translate":"text.lezah.spectator_mode1","color": "yellow","bold": true},{"translate":"text.lezah.spectator_mode2","color": "white","bold": false}]
function lezah:old_content_folder/time
function lezah:old_content_folder/tips
#function lezah:mechanics/main
function lezah:nbt/main
function lezah:settings/main