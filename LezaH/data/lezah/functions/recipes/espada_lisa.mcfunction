advancement revoke @s only lezah:onlyrecipes/espada_lisa
recipe take @s lezah:espada_lisa

execute store result score @s crafteos run clear @s command_block

execute if score @s crafteos matches 1.. run summon item ~ ~ ~ {Tags:["tagtemporal"],Item:{id:"stone_sword",Count:1b,tag:{liso:1,display:{Name:'[{"translate":"item.lezah.smooth_stone_sword","italic":false}]'},"item_propose":"general"}}}
