advancement revoke @s only lezah:onlyrecipes/azada_lisa
recipe take @s lezah:azada_lisa

execute store result score @s lezah.Craft run clear @s command_block

execute if score @s lezah.Craft matches 1.. run summon item ~ ~ ~ {Tags:["tagtemporal"],Item:{id:"stone_hoe",Count:1b,tag:{liso:1,display:{Name:'[{"translate":"item.lezah.smooth_stone_hoe","italic":false}]'}}}}
